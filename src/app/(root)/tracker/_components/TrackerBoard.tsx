...
const textColor = ["text-black","text-blue","text-orange","text-green"]
export default function TrackerBoard() {
  ...
  const initializeTasks = () => {
    const initialTasks = user?.job_statuses.reduce(
      (acc: Tasks, statusObj: { label: string; value: number }) => {
        acc[statusObj.label] = [];
        return acc;
      },
      {}
    ...
    setColumnInputValues(
      user?.job_statuses.map((status) => status.label) || []
    );
  };
  ...
  const fetchJobs = async () => {
    try {
      ...
      if (Array.isArray(data?.jobs)) {
        const organizedJobs = user?.job_statuses.reduce(
          (acc: Tasks, statusObj: { label: string; value: number }) => {
            acc[statusObj.label] = data.jobs.filter(
              (job: Job) => job.status === statusObj.label
            );
            return acc;
          },
          {}
  ...
  const handleRemoveColumn = (index: number, name: string) => {
    ...
    const updatedColumnInputValues = columnInputValues.filter(
      (_, i) => i !== index
    );
    setColumnNames(updatedColumnNames);
            ...
            {columnNames.map((listName, index) => (
              <Droppable droppableId={listName} key={listName}>
                {(provided) => (
                  ...
                  >
                    <div className={`flex items-center justify-between space-x-2 ${textColor[index % textColor.length]} font-medium mb-2`}>
                      ...
                      <input
                        ...
                        className="font-bold capitalize bg-transparent focus:bg-gray-200 focus:p-1"
                      />
                      <button
                    ...
                    {tasks[listName]?.length === 0 && (
                      <div className="inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        <button
